...
function B1Card({ children, style, onClick, glow, pad = 16, className }) {
  ...
  return React.createElement('div', {
    ...
    style: {
      ...
      transition: 'all 0.34s cubic-bezier(0.16,1.18,0.28,1)',
      transform:`perspective(900px) rotateX(var(--cortex-card-tilt-y, 0deg)) rotateY(var(--cortex-card-tilt-x, 0deg)) translate3d(var(--cortex-card-shift-x, 0px), var(--cortex-card-shift-y, 0px), 0) scale(var(--cortex-motion-scale, 1)) scale(var(--cortex-shake-scale, 1)) scale(${p && onClick ? 0.97 : 1})`,
...
function B1Button({ label, icon, onClick, variant = 'primary', full, style }) {
  ...
  return React.createElement('div', {
    ...
    style: {
      ...
      transition:'all 0.3s cubic-bezier(0.16,1.18,0.28,1)',
      transform:`perspective(700px) rotateX(var(--cortex-button-tilt-y, 0deg)) rotateY(var(--cortex-button-tilt-x, 0deg)) translate3d(var(--cortex-button-shift-x, 0px), var(--cortex-button-shift-y, 0px), 0) scale(var(--cortex-motion-scale, 1)) scale(var(--cortex-shake-scale, 1)) scale(${p ? 0.95 : 1})`,
      width: full ? '100%' : 'auto',
      ...style,
    }
  }, icon, label);
