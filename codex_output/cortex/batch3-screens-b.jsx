...
function PageProviderAdvancedSettings({ onNavigate }) {
  ...
  return React.createElement(B2PageShell, {
    ...
    right:React.createElement(B1Badge, { color:providerEnabled ? DL.teal : DL.gold }, providerEnabled ? 'Proxy path' : 'No secret'),
  },
    React.createElement(B3Hero, {
      kicker:'Provider architecture',
      title:'Model Provider and Tactile Controls',
      subtitle:'Provider wiring now follows a secure-proxy rule: secrets stay off GitHub Pages, while the shell stays ready for compliant runtime attachment.',
      color:DL.gold,
    }),

    React.createElement(B1Section, { title:'Provider', style:{ marginBottom:9 } }),
    React.createElement(B1Card, { pad:14, style:{ marginBottom:14 }},
      React.createElement(B1Toggle, { value:providerEnabled, onChange:setProviderEnabled, label:'Enable secure proxy route' }),
      React.createElement(B3Row, {
        title:'Provider slot',
        sub:'No real API key is stored in the client. Local and production secrets belong behind a proxy or server runtime.',
        right:React.createElement(B1Badge, { color:DL.green }, 'Secret-safe'),
      }),
      React.createElement(B3Row, {
        title:'Capability status',
        sub:'Public shell: safe. Proxy wiring: ready. Direct Coding Plan app calls: discouraged until replaced by a compliant backend path.',
        active:true,
        color:DL.teal,
        right:React.createElement(B1Badge, { color:DL.teal }, 'Proxy-ready'),
      }),
      React.createElement(B3Row, {
        title:'Runtime target',
        sub:'Recommended base route is a proxy in front of ModelArk standard API endpoints. GitHub Pages should never expose the private provider credential.',
        right:React.createElement(B1Badge, { color:DL.accentB }, 'Compliant'),
      }),
    ),
